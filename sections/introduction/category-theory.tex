...
\begin{document}
    ...
    \begin{example}
        ...
        The category of topological spaces, usually denoted $\cat{Top}$, has objects being topological spaces and morphisms being continuous functions. It is also a subcategory of $\cat{Set}$. A related example is $\cat{hTop}$ with the same objects but homotopy equivalence classes as morphisms. Yet another related category would be $\cat{Haus}$ where instead the objects are topological spaces with the Hausdorff property and continuous functions as morphisms.
    \end{example}
