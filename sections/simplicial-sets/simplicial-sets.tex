...
\begin{document}
    ...
    \begin{corollary}\label{uni-sim}
        ...
        \[x' = s_{j_h},...,s_{j_1}x\]
        where x is a non-degenerate simplex in X and the subscripts j satisfy $j_1 <... < j_h$.
    \end{corollary}
    ...
    The $d^i,\; s^i$ are the induced coface and codegeneracy maps. Note that should one abandon the second equality relation one gets the fat geometric realization, an, for this report, esoteric concept. For reference $\bigGeom X = \coprod_{n=0}^{\infty} X_n \times \geom[\Delta^n]$. A point $(x_n,u_n)\in \bigGeom X$ is called \defemph{non-degenerate} if $x_n$ is non-degenerate and $u_n$ is an interior point of the simplex defined in the algebraic topology section, i.e. $u_n\in \Delta_n$ is interior if $ n=0$ or $ 0< t_i < 1 \;\forall\; i$. A proof from \cite{simp-maye} follows.
    ...
    For simplicial sets the realization as an topological object is complicated by the degenerate simplices. This is bypassed by finding an equivalent non-degenerate point of each point and done by the help of lemma \ref{deg-rep}.
    ...
    In simple terms the proof gathers from the proof of proposition \ref{is-CW} where the constructed function is used to get representatives and then establishing a bijection. 
