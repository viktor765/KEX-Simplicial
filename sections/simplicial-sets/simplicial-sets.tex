...
\begin{document}
    ...
    \begin{definition}
        ...
        \begin{itemize}
            \item $(d_ik_{n},u_{n-1})\sim(k_{n}, d^iu_{n-1})$, $k_n\in K_n,\; u_{n-1} \in \Delta _{n-1}$
            \item $(s_ik_n, u_{n+1})\sim(k_n, s^iu_{n+1})$, $k_n \in K_n,\; u_{n+1}\in \Delta_{n+1}$
        \end{itemize}
    \end{definition}
    
    The $d^i,\; s^i$ are the induced coface and codegeneracy maps. Note that should one abandon the second equality relation one gets the fat geometric realization, an, for this report, uninteresting concept.  
